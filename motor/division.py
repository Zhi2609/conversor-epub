...
RE_ENCABEZADO_VACIO = re.compile(r'<h[1-3](?:\s[^>]*)?>(?:\s|&nbsp;)*</h[1-3]>', re.IGNORECASE)
...
def dividir_en_capitulos(
    ...
    matches = [m for m in RE_ENCABEZADO.finditer(html) if texto_plano(m.group(2))]

    if not matches:
        titulo = titulos[0] if titulos else f'Capítulo {start_num}'
        return [Chapter(titulo=titulo, html_cuerpo=RE_ENCABEZADO_VACIO.sub('', html).strip())]

    capitulos: list[Chapter] = []
    contador = start_num

    pre = RE_ENCABEZADO_VACIO.sub('', html[:matches[0].start()]).strip()
    if pre:
    ...
    for i, match in enumerate(matches):
        cuerpo = RE_ENCABEZADO.sub('', html[limites[i]:limites[i + 1]], count=1)
        cuerpo = RE_ENCABEZADO_VACIO.sub('', cuerpo).strip()
