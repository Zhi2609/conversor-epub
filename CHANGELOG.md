...
- **Bundle 100% Autónomo**: La compilación final (`build/linux/x64/release/bundle/`) ahora contiene todo lo necesario dentro de su propio directorio. Se puede comprimir y compartir directamente sin carpetas o scripts externos.
      ...
      21:30 — Desduplicación Inteligente de Títulos, Múltiples Especiales e Interludios
- **Desduplicación Inteligente de Títulos**: Se implementó `descomponerTitulo` en `plantillas.dart` para separar automáticamente prefijos ordinales (`Capítulo 1:`) del subtítulo (`De Encantador a Espadachín`). Al renderizar en `template.xhtml`, se previene la duplicación redundante en `<h1 title="...">` y en el subtítulo versalita.
- **Múltiples Prólogos y Epílogos**: `clasificarYRenumerarCapitulos` ahora numera automáticamente prólogos y epílogos múltiples (`prologo_01.xhtml`, `prologo_02.xhtml`, etc.), preservando la plantilla especial para ambos y evitando que consuman números de capítulos ordinarios.
